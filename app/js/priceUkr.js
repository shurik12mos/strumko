;($(window).ready(function(){
	...
	$.getJSON("../api/pricelist.json", function(data){		
	    $.each(data, function(i,item){	  
			if (i!=0 && item["разделукр"] == data[i-1]["разделукр"]) {
				var str = "<tr><td>"+item['Наименованиеукр']+"</td><td>"+item['еденицы измерения']+"</td><td>"+item['Стоимость']+"</td></tr>";
				$('.table').append(str);  
			}else {
				var str = "<tr><td colspan='3'>" + item["разделукр"] + "</td></tr>";
				var str = str + "<tr><td>"+item['Наименованиеукр']+"</td><td>"+item['еденицы измерения']+"</td><td>"+item['Стоимость']+"</td></tr>";
				$('.table').append(str);
			}	
